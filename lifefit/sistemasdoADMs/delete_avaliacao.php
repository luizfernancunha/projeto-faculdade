...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['id'])) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                if ($stmt->affected_rows > 0) {
                    ...
                    $response['message'] = 'Registro de avaliação excluído com sucesso.';
                } else {
                    $response['message'] = 'Nenhum registro de avaliação encontrado com o ID fornecido.';
                }
            } else {
        ...
        $response['message'] = 'ID da avaliação não fornecido para exclusão.';
    }
} else {
